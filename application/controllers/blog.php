<?php

class Blog extends CI_Controller
{   
    function __construct() 
    {
        parent::__construct();
        $this->load->model('m_db');
    }
    function index($start = 0)//index page
    {
        $data['posts'] = $this->m_db->get_posts(5, $start);
        
        //pagination
        $this->load->library('pagination');
        $config['base_url'] = base_url().'blog/index/';//url to set pagination
        $config['total_rows'] = $this->m_db->get_post_count();
        $config['per_page'] = 5; 
        $this->pagination->initialize($config); 
        $data['pages'] = $this->pagination->create_links(); //Links of pages
        
        $class_name = array(
            'home_class'=>'active',
            'login_class' => '', 
            'register_class' => '',
            'upload_class'=>'',
            'news_class'=>'',
            'contact_class'=>'');
        
        $this->load->model('m_db');
        $id=1;
        $veri['hakkimizda']=$this->m_db->get_getir($id);
        $merge = array_merge($veri, $class_name);
        $this->load->view('header',$merge);
        $this->load->view('v_home',$data);
        $this->load->view('footer');
    }
    function post($post_id)//single post page
    {
        $data['posts'] = $this->m_db->get_posts(5);
        $this->load->model('m_comment');
        $data['comments'] = $this->m_comment->get_comment($post_id);    
        $data['post'] = $this->m_db->get_post($post_id);
        $class_name = array(
            'home_class'=>'active',
            'login_class' =>'', 
            'register_class' => '',
            'upload_class'=>'',
            'news_class'=>'',
            'contact_class'=>'');
        $this->load->model('m_db');
        $id=1;
        $veri['hakkimizda']=$this->m_db->get_getir($id);
        $merge = array_merge($veri, $class_name);
        $this->load->view('header',$merge);
        $this->load->view('v_single_post',$data);
        $this->load->view('footer');
    }
    
    function new_post()//Creating new post page
    {
        $data['posts'] = $this->m_db->get_posts(5);
        if(!$this->check_permissions('author'))//when the user is not an andmin and author
        {
            redirect(base_url().'users/login');
        }
        if($this->input->post())
        {
            $data = array(
                'post_title' => $this->input->post('post_title'),
                'post' => $this->input->post('post'),
                'active' => 1,
				'image_path' => 'oguz/uploads/images/'.$this->resim_yukle("images"),
            );
            $this->m_db->insert_post($data);
            redirect(base_url().'blog/');
        }
        else{
            
            $class_name = array(
            'home_class'=>'active',
            'login_class' =>'', 
            'register_class' => '',
            'upload_class'=>'',
            'news_class'=>'',
            'contact_class'=>'');
            $this->load->model('m_db');
            $id=1;
            $veri['hakkimizda']=$this->m_db->get_getir($id);
            $merge = array_merge($veri, $class_name);
            $this->load->view('header',$merge);
            $this->load->view('v_new_post',$data);
            $this->load->view('footer');
			
			
			
        }
    }
    function editpost($post_id)//Edit post page
    {
        $data['posts'] = $this->m_db->get_posts(5);
        if(!$this->check_permissions('author'))//when the user is not an andmin and author
        {
            redirect(base_url().'users/login');
        }
        $data['success'] = 0;
        
        if($this->input->post())
        {
            $data = array(
                'post_title' => $this->input->post('post_title'),
                'post' => $this->input->post('post'),
                'active' => 1
            );
            $this->m_db->update_post($post_id, $data);
            $data['success'] = 1;
        }
        $data['post'] = $this->m_db->get_post($post_id);
        
        $class_name = array(
            'home_class'=>'active',
            'login_class' =>'', 
            'register_class' => '',
            'upload_class'=>'',
            'news_class'=>'',
            'contact_class'=>'');
        $this->load->model('m_db');
        $id=1;
        $veri['hakkimizda']=$this->m_db->get_getir($id);
        $merge = array_merge($veri, $class_name);
        $this->load->view('header',$merge);
        $this->load->view('v_edit_post',$data);
        $this->load->view('footer');
    }
	
    function editbilgi($user_id)//Edit post page
    {
        $dene['posts'] = $this->m_db->get_posts(5);
        $data['success'] =0;
        
        if($this->input->post())
        {
            $data = array(
                'username' => $this->input->post('username'),
                'email' => $this->input->post('email')
            );
            $this->m_db->update_profil($user_id, $data);
            $data['success'] = 1;
        }
        $data['post'] = $this->m_db->get_bilgi($user_id);
        
        $class_name = array(
            'home_class'=>'', 
            'login_class' =>'', 
            'register_class' => 'active',
            'upload_class'=>'',
            'news_class'=>'',
            'contact_class'=>'');
        $this->load->model('m_db');
        $id=1;
        $veri['hakkimizda']=$this->m_db->get_getir($id);
        $merge = array_merge($veri, $class_name);
        $birles=array_merge($merge,$dene);
        $this->load->view('header',$birles);
        $this->load->view('v_edit_profil',$data);
        $this->load->view('footer');
    }
	
	
    function deletepost($post_id)//delete post page
    {
        if(!$this->check_permissions('author'))//when the user is not an andmin and author
        {
            redirect(base_url().'users/login');
        }
        $this->m_db->delete_post($post_id);
        redirect(base_url().'blog/');
    }
	
	function resim_yukle($data){          
			$config['upload_path'] = "oguz/uploads/$data/"; // dosyayi upload edecegimiz klasör
			$config['allowed_types'] = 'gif|jpg|jpeg|png';
			$this->load->library('upload', $config); 
			$sonuc = $this->upload->do_upload('image_path'); 
			$upload_data = $this->upload->data(); 
			$file_name =$upload_data['file_name'];
			return $file_name;
		}
    
    function check_permissions($required)//checking current user's permission
    {
        $user_type = $this->session->userdata('user_type');//curren user
        if($required == 'user')//requirment is user 
        {
            if($user_type){return TRUE;}//all user have permission
        }
        elseif($required == 'author')//when requirement is author
        {
            if($user_type == 'author' || $user_type == 'admin')//author and admin have the permission
            {
                return TRUE;
            }
        }
        elseif($required == 'admin')//when required is admin
        {
            if($user_type == 'admin'){return TRUE;}//only admin have the permission
        }
    }
	
	
}
